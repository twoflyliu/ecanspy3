...
public:
    ...
    virtual void SetContTxCount(UINT cont_tx_cnt)
    {
        (void)cont_tx_cnt;
    }
...
class MessageTriggerTx: public MessagePeriodTx
{
public:
    enum {
        DEFAULT_CONT_TX_CNT = 3,
    };

    MessageTriggerTx(UINT id, UINT len, UINT period, UINT offset,
                     UINT cont_tx_cnt = DEFAULT_CONT_TX_CNT);
    void SetContTxCount(UINT cont_tx_cnt);

protected:
    void OnPostTransmit() override;

private:
    UINT cont_tx_cnt_;      //!< 连续发送次数
    UINT curr_tx_cnt_;
};
...
public:
    ...
    void AddMessageTriggerTx(UINT id, UINT len, UINT period, UINT offset,
                             UINT cont_tx_cnt = MessageTriggerTx::DEFAULT_CONT_TX_CNT);
    ...
    void SetContTxCount(UINT cont_tx_cnt);
    void SetContTxCount(UINT can_id, UINT cont_tx_cnt);
...
class MultiMediaMessagePeriodTxMgr 
{
public:
	enum {
	...
	static MultiMediaMessagePeriodTxMgr &GetInstance()
	{
		static MultiMediaMessagePeriodTxMgr instance(MULTI_MEDIA_MESSAGE_PERIOD);
		return instance;
	}

	~MultiMediaMessagePeriodTxMgr()
	{
...
private:
	MultiMediaMessagePeriodTxMgr(UINT period, UINT timer_res=1) : message_period_tx_mgr_(MessagePeriodTxMgr::GetInstance())
	{
